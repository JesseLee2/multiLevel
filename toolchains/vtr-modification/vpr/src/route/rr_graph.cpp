...
static void alloc_and_load_rr_graph_gsb(const int num_nodes,
                                        ...
                                        t_two_stage_mux_map* imux_two_stage_mux_map,
                                        t_two_stage_mux_map* omux_two_stage_mux_map,
...
void create_rr_graph(const t_graph_type graph_type,
    ...
    if (!det_routing_arch->read_rr_graph_filename.empty()) {
        ...
        if(WRTIE_GSB_ARCH){
            write_one_gsb_arch("gsb_arch_check.txt", 10, 10, segment_inf);
        }
...
static void build_rr_graph(const t_graph_type graph_type,
    ...
    *Warnings = RR_GRAPH_NO_WARN;
    ...
    t_medium_seg_inf medium_seg_inf;
    std::map<int, int> clbpin_medium_map;//find medium node for imux. {clb_pin_ind->imux_medium_ind},每个port是连续的
    std::vector<std::map<std::string, int>> mux_name_medium_map_vec;//0-imux, 1-gsb
    //std::vector<int> gsb_medium_id_vec;
    ...
    if (is_global_graph) {
        ...
        //max_chan_width记录的是不含中间节点的宽度，num_seg_details记录的是所有seg_detail的数量，含中间节点
    ...
    if (num_detailics > 0) {
        pt_to_pt_detailICs = alloc_and_load_pt_to_pt_detailICs(detailICs, num_detailics, 
                                                                delayless_switch, segment_inf, nums_per_segtype,
                                                                &nodes_per_chan, chan_details_x, chan_details_y);
    }
    /*END PT_TO_PT */
    ...
    /* START GSB ARCH*/
    //构建其余pb和第一种pb的引脚映射关系
    //pbtype_name -> i*height+j -> {clbpin -> pbpin}
    std::map<std::string, std::vector<std::map<int, int>>> other_pbpin_to_clbpin_map;
    build_other_pbpin_to_clbpin(arch->gsb_inf[0], other_pbpin_to_clbpin_map); //默认只有一种gsb

    
    t_gsb_connection_map* gsb_conn_map = nullptr;//for gsb arch
    ...
    t_two_stage_mux_map* imux_stage_mux_map = nullptr;
    t_two_stage_mux_map* omux_stage_mux_map = nullptr;
    std::pair<t_two_stage_mux_map*, t_two_stage_mux_map*> two_stage_mux_map;
    if(arch->gsb_inf[0].twostage_mux_spec){
        two_stage_mux_map = alloc_and_load_two_stage_map(chan_details_x, chan_details_y, grid, arch->gsb_inf, arch->imux_inf,
                                                              segment_inf, &nodes_per_chan, other_pbpin_to_clbpin_map, mux_name_medium_map_vec[1],
                                                              mux_name_medium_map_vec[0], mux_name_medium_map_vec[2], clbpin_medium_map, wire_to_arch_ipin_switch);
        gsb_stage_mux_map = two_stage_mux_map.first;
    }else{
    ...
    t_gsb_connection_map* imux_conn_map = nullptr;
    if(arch->imux_inf[0].twostage_mux_spec){
        imux_stage_mux_map = two_stage_mux_map.second;
        if(imux_stage_mux_map==nullptr){
            two_stage_mux_map = alloc_and_load_two_stage_map(chan_details_x, chan_details_y, grid, arch->gsb_inf, arch->imux_inf,
                                                             segment_inf, &nodes_per_chan, other_pbpin_to_clbpin_map, mux_name_medium_map_vec[1],
                                                             mux_name_medium_map_vec[0], mux_name_medium_map_vec[2], clbpin_medium_map, wire_to_arch_ipin_switch);
            imux_stage_mux_map = two_stage_mux_map.second;
        ...
        imux_conn_map = alloc_and_load_imux_permutations(chan_details_x, chan_details_y, grid, arch->imux_inf, &nodes_per_chan, wire_to_arch_ipin_switch);
    }

    t_omux_connection_map* omux_conn_map = nullptr;
    if(arch->omux_inf[0].twostage_mux_spec){
        omux_stage_mux_map = alloc_and_load_omux_two_stage_map(chan_details_x, chan_details_y, grid, arch->omux_inf,
                                                          segment_inf, &nodes_per_chan, other_pbpin_to_clbpin_map, mux_name_medium_map_vec[2],
                                                          clbpin_medium_map);
    } else{
        omux_conn_map = alloc_and_load_omux_permutations(arch->omux_inf);
    }
    ...
    alloc_and_load_rr_graph_gsb(device_ctx.rr_nodes.size(), device_ctx.rr_nodes, segment_inf.size(),
                                ...
                                gsb_conn_map, imux_conn_map, omux_conn_map,
                                gsb_multistage_mux_map,
                                gsb_stage_mux_map, imux_stage_mux_map, omux_stage_mux_map,
    ...
    int medium_num = medium_seg_inf.imux_mdeium_num_xory + medium_seg_inf.omux_mdeium_num_xory + medium_seg_inf.gsb_mdeium_num_xory;
    ...
    if (imux_conn_map) {
        free_permutations(imux_conn_map);
        imux_conn_map = nullptr;
    }
    if(gsb_stage_mux_map){
        free_permutations(gsb_stage_mux_map);
        gsb_stage_mux_map = nullptr;
    }
    if(imux_stage_mux_map){
        free_permutations(imux_stage_mux_map);
        imux_stage_mux_map = nullptr;
    }
    if (omux_conn_map) {
        free_permutations(omux_conn_map);
        //free_permutations<t_omux_connection_map>(omux_conn_map);
        omux_conn_map = nullptr;
    }
    ...
    */
    ...
    for (size_t i = 0; i < grid.width() - 1; ++i) {
        for (size_t j = 0; j < grid.height() - 1; ++j) {
            ...
            if(is_gsb_arch){
                ...
                for (auto gsb_pbtype_name : arch->gsb_inf[0].pbtype_names) {//默认只有一种GSB
    ...
    for (size_t i = 0; i < grid.width() - 1; ++i) {
        for (size_t j = 0; j < grid.height() - 1; ++j) {
            ...
            if(record_imx_coord.size() == arch->imux_inf.size()) break;
        }
        if(record_imx_coord.size() == arch->imux_inf.size()) break;
    }
    ...
    for (size_t i = 0; i < grid.width() - 1; ++i) {
        for (size_t j = 0; j < grid.height() - 1; ++j) {
            int edge_count = build_gsb_arch(i, j, record_imx_coord, grid, chan_details_x, chan_details_y,
                                            ...
                                            gsb_conn_map, imux_conn_map, omux_conn_map,
                                            gsb_two_stage_mux_map, imux_two_stage_mux_map, omux_two_stage_mux_map,
...
static int build_rr_chan_to_ipin(const int x_coord,
    ...
    for (int track = 0; track < tracks_per_chan; ++track) {
        ...
        //fc是0，对于gsb块没影响
        //对于非gsb块，因为只考虑起点在此位置的的线，因此起点不再此位置的线如果有连到此位置ipin，可能实际没有连接，因此连接数要比实际fc小
        int num_edges = 0;
...
static void init_rr_node_gsb_arch(const int x_coord,
    ...
    for(int i = 0; i < 3; i++) {
        int num_medium = i == 0 ? medium_seg_inf.imux_mdeium_num_xory :
                         (i == 1 ? medium_seg_inf.omux_mdeium_num_xory : medium_seg_inf.gsb_mdeium_num_xory);
        ...
        for (int i_m = 0; i_m < num_medium; ++i_m) {
            ...
            /*if(i == 2){
                actual_gsb_medium_num = chan_type == CHANX ? 0 : num_medium;
                L_rr_node[node].set_medium_id(gsb_medium_id_vec[actual_gsb_medium_num + i_m]);
            }*/
        }
...
static int build_gsb_arch(const int x_coord,
    ...
    if(imux_conn_map != nullptr){
        //imux seg
        edge_count += get_track_to_imux_for_imux(x_coord, y_coord, grid, chan_details_x, chan_details_y, L_rr_node_indices, L_medium_rr_node_indices,
                                                    rr_edges_to_create, imux_conn_map, clbpin_to_medium, other_pbpin_to_clbpin_map);

        //imux omux
        edge_count += get_omuxOrpb_to_imux_for_imux(x_coord, y_coord, record_imux_coord, grid, L_rr_node_indices, L_medium_rr_node_indices,
                                                    rr_edges_to_create, imux_conn_map, clbpin_to_medium, other_pbpin_to_clbpin_map, actual_pbtype_name, e_from_type::FT_OMUX);

        //imux pb
        edge_count += get_omuxOrpb_to_imux_for_imux(x_coord, y_coord, record_imux_coord, grid, L_rr_node_indices, L_medium_rr_node_indices,
                                                    rr_edges_to_create, imux_conn_map, clbpin_to_medium, other_pbpin_to_clbpin_map, actual_pbtype_name, e_from_type::FT_PB);

        //imux imux
        edge_count += get_omuxOrpb_to_imux_for_imux(x_coord, y_coord, record_imux_coord, grid, L_rr_node_indices, L_medium_rr_node_indices,
                                                    rr_edges_to_create, imux_conn_map, clbpin_to_medium, other_pbpin_to_clbpin_map, actual_pbtype_name, e_from_type::FT_IMUX);
    }
    else{
        VTR_ASSERT(imux_two_stage_mux_map != nullptr);
        edge_count += build_two_stage_mux_for_gsb(x_coord, y_coord, rr_edges_to_create, imux_two_stage_mux_map);
    }
    ...
    if(omux_conn_map != nullptr){
        ...
        VTR_ASSERT(omux_two_stage_mux_map != nullptr);
        edge_count += build_two_stage_mux_for_gsb(x_coord, y_coord, rr_edges_to_create, omux_two_stage_mux_map);
