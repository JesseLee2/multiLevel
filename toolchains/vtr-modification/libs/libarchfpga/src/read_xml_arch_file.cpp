...
static void ProcessFrom(pugi::xml_node From_Node, t_common_from_inf& com_f, const pugiutil::loc_data& loc_data, ReqOpt type_req = ReqOpt::REQUIRED);
...
static void ProcessImuxFrom(pugi::xml_node From_Node, t_imux_from_inf& imx_f, const pugiutil::loc_data& loc_data);
static void ProcessIMUX(pugi::xml_node IMux_Node, t_arch* arch, const pugiutil::loc_data& loc_data);
static void ProcessGsbFrom(pugi::xml_node From_Node, t_gsb_from_inf& gsb_f, const pugiutil::loc_data& loc_data);
...
//for GSB Architecture
static void ProcessFrom(pugi::xml_node From_Node, t_common_from_inf& com_f, const pugiutil::loc_data& loc_data, ReqOpt type_req) {
    const char* tmp;
    ReqOpt total_froms_req = OPTIONAL;
    tmp = get_attribute(From_Node, "type", loc_data, type_req).as_string();
    com_f.from_type = string_to_fromtype(tmp);
    if(com_f.from_type != e_from_type::FT_SEGMENT){
        total_froms_req = REQUIRED;
    }

    tmp = get_attribute(From_Node, "name", loc_data, type_req).as_string();
    if(tmp){
        auto names = vtr::split(tmp," ");
        for (const auto& name : names) {
            com_f.from_name.push_back(name);
        }
    }
    
    com_f.total_froms = get_attribute(From_Node, "total_froms", loc_data, total_froms_req).as_int(999);//one large number represents all
    com_f.from_start = get_attribute(From_Node, "from_start", loc_data, OPTIONAL).as_int(0);
    com_f.num_foreach = get_attribute(From_Node, "num_foreach", loc_data, OPTIONAL).as_int(1);
    com_f.offset = get_attribute(From_Node, "offset", loc_data, OPTIONAL).as_int(0);
    com_f.from_group = get_attribute(From_Node, "from_group", loc_data, OPTIONAL).as_int(1);
    com_f.to_group = get_attribute(From_Node, "to_group", loc_data, OPTIONAL).as_int(1);

    if(com_f.from_start != 0 && com_f.from_group != 1){
        archfpga_throw(loc_data.filename_c_str(), loc_data.line(From_Node),
                       "from_group should be 1 with from_start != 0.\n");
    }

    tmp = get_attribute(From_Node, "from_order", loc_data, OPTIONAL).as_string();
    com_f.pos_order = tmp == std::string("neg") ? false : true;

    if(com_f.from_type == e_from_type::FT_SEGMENT) {
        tmp = get_attribute(From_Node, "from_switchpoint", loc_data, OPTIONAL).as_string(nullptr);
        if(tmp){
            auto fsps = vtr::split(tmp," ");
            for (const auto& fsp : fsps) {
                com_f.from_switchpoints.push_back(vtr::atoi(fsp));
            }
        }
        else{
            com_f.from_switchpoints.push_back(0);
        }
        /*
        for(auto const ai : com_f.from_switchpoints){
            VTR_LOG("%d",ai);
        }*/
    }
    else{
        tmp = get_attribute(From_Node, "pin_types", loc_data, OPTIONAL).as_string(nullptr);
        if(tmp){
            auto pts = vtr::split(tmp," ");
            for (const auto& pt : pts) {
                com_f.pin_types.push_back(pt);
            }
        }
        else{
            com_f.pin_types.push_back(std::string("all"));
        }
    }
...
static void ProcessImuxFrom(pugi::xml_node From_Node, t_imux_from_inf& imx_f, const pugiutil::loc_data& loc_data) {
    ...
    itmp = get_attribute(From_Node, "only_detail", loc_data, OPTIONAL).as_int(0);
    imx_f._only_detail = itmp ? true : false;

    expect_only_children(From_Node, {"detail"}, loc_data);
    int num_details = count_children(From_Node, "detail", loc_data, OPTIONAL);
    imx_f._imux_detail_inf.clear();
    Node = get_first_child(From_Node, "detail", loc_data, OPTIONAL);
    for (int i_d = 0; i_d < num_details; i_d++) {
        std::vector<t_detail_from_inf> detail_froms;
        ProcessFromDetail(Node, detail_froms, loc_data);
        imx_f._imux_detail_inf.insert(imx_f._imux_detail_inf.end(), detail_froms.begin(), detail_froms.end());
        Node = Node.next_sibling(Node.name());
    }
    /*
    if(imx_f.only_detail){
        tmp = get_attribute(From_Node, "type", loc_data).as_string();
        imx_f.from_type = string_to_fromtype(tmp);
        tmp = get_attribute(From_Node, "name", loc_data).as_string();
        if(tmp){
            auto names = vtr::split(tmp," ");
            for (const auto& name : names) {
                imx_f.from_name.push_back(name);
            }
        }
        return;
    }
    */
    ...
    tmp = get_attribute(From_Node, "to_fix_order", loc_data, OPTIONAL).as_string(nullptr);
    if(tmp){
        std::string ss = tmp;
        imx_f._to_fix_order.reserve(ss.length());
        //VTR_LOG("%d",ss.size());
        for (const char ch : ss) {
            //int ii=vtr::atoi(std::string(&ch));
            imx_f._to_fix_order.push_back(vtr::atoi(std::string(1,ch)));
        }
    }
    imx_f._overlap = get_attribute(From_Node, "overlap", loc_data, OPTIONAL).as_int(0);
    ProcessFrom(From_Node, imx_f, loc_data);
    return;
}
...
static void ProcessFirstStage(pugi::xml_node Stage_Node, std::vector<t_firststage_mux_inf>& first_stages, const pugiutil::loc_data& loc_data) {
    ...
    int num_mux = count_children(Stage_Node, "mux", loc_data, OPTIONAL);
    first_stages.reserve(num_mux);
    Node = get_first_child(Stage_Node, "mux", loc_data, OPTIONAL);
    for (int i_m = 0; i_m < num_mux; i_m++) {
        t_firststage_mux_inf fm;
        fm.mux_name = get_attribute(Node, "name", loc_data).as_string();
        //fm.medium_id = get_attribute(Node, "medium_id", loc_data, OPTIONAL).as_int(0);
        ...
        for (int i_f = 0; i_f < num_froms; i_f++) {
            ...
            for (auto const& detail_str : detail_strs) {
                if (mux_same_f.mux_from_type == e_from_type::FT_SEGMENT) {
                    ...
                    mux_same_f.from_omux.push_back(detail_str);
                    mux_same_f.from_pin_types.push_back("");
                    ...
                    mux_same_f.from_inds.push_back(std::stoi(pin_strs[1]));
                    mux_same_f.from_pin_types.push_back(pin_strs[0]);
                } else if(mux_same_f.mux_from_type == e_from_type::FT_CAS){
                    mux_same_f.from_sides.push_back(NUM_SIDES);
                    mux_same_f.from_cas.push_back(detail_str);
                    mux_same_f.from_pin_types.push_back("");
                } else if(mux_same_f.mux_from_type == e_from_type::FT_zero) {
                    mux_same_f.from_sides.push_back(NUM_SIDES);
                    mux_same_f.from_zero.push_back(detail_str);
                    mux_same_f.from_pin_types.push_back("");
                } else{
                    archfpga_throw(loc_data.filename_c_str(), loc_data.line(SubElem),
                                   "Error from_type!!!\n");
...
static void ProcessSecondStage(pugi::xml_node Stage_Node, std::vector<t_secondstage_mux_inf>& second_stages, const pugiutil::loc_data& loc_data) {
    ...
    for (int i_m = 0; i_m < num_mux; i_m++) {
        ...
        //gsb
        tmp = get_attribute(Node, "to_seg_name", loc_data, OPTIONAL).as_string(nullptr);
        if(!is_imux && tmp){
        ...
        //cas in gsb and omux
        if(!is_imux && !tmp){
            sm.to_name = get_attribute(Node, "name", loc_data).as_string();
            sm.to_side = NUM_SIDES;
            //auto pin_strs = vtr::split(sm.mux_name, "-");
            //sm.to_ind = std::stoi(pin_strs[1]);
            sm.to_pin_name = "";
        }
        ...
        for (int i_f = 0; i_f < num_froms; i_f++) {
            ...
            if(tmp){
                sm.firststage_mux_name = vtr::split(tmp, " ");
                SubElem = SubElem.next_sibling(SubElem.name());
                continue;
            }
...
static void ProcessIMUX(pugi::xml_node IMux_Node, t_arch* arch, const pugiutil::loc_data& loc_data) {
    ...
    tmp = get_attribute(IMux_Node, "name", loc_data).as_string(nullptr);
    if (tmp) {
        imx.name = tmp;
    }
    else {
        archfpga_throw(loc_data.filename_c_str(), loc_data.line(IMux_Node),
                        "No name specified for the imux.\n");
    }

    tmp = get_attribute(IMux_Node, "pbtype_name", loc_data).as_string(nullptr);
    if (tmp) {
        imx.pbtype_names = vtr::split(tmp, " ");
    }
    else {
        archfpga_throw(loc_data.filename_c_str(), loc_data.line(IMux_Node),
                        "No pb_type specified for the imux.\n");
    }

    tmp = get_attribute(IMux_Node, "arch_imux_switch", loc_data).as_string(nullptr);
    if (tmp) {
        /* Match names */
        int j;
        ...
        imx.arch_imux_switch = j;
    }
    else {
        archfpga_throw(loc_data.filename_c_str(), loc_data.line(IMux_Node),
                        "No arch_omux_switch specified for the imux.\n");
    }
    
    imx.lut_group = get_attribute(IMux_Node, "lut_group", loc_data, OPTIONAL).as_int(1);

    expect_only_children(IMux_Node, {"group", "multistage_muxs"}, loc_data);

    //<omux> flag
    int num_groups = count_children(IMux_Node, "group", loc_data);
    VTR_ASSERT(imx.lut_group == num_groups);
    imx.imux_group_inf.reserve(num_groups);
    Node = get_first_child(IMux_Node, "group", loc_data);
    for (int i_g = 0; i_g < num_groups; i_g++) {
        /* use a temp variable which will be assigned to omux later */
        t_imux_group_inf imx_g;

        /* get name */
        tmp = get_attribute(Node, "name", loc_data).as_string(nullptr);
        if (tmp) {
            imx_g.lut_name = vtr::split( tmp , " ");
        }
        else {
            archfpga_throw(loc_data.filename_c_str(), loc_data.line(Node),
                            "No name specified for the imux group #%d.\n", i_g);
        }

        itmp = get_attribute(Node, "mux_nums", loc_data).as_int();
        if (itmp) {
            imx_g.mux_nums_eachlut = itmp;
            //验证itmp和clb中的lut输入是否相等；
        }
        else {
            archfpga_throw(loc_data.filename_c_str(), loc_data.line(Node),
                            "No mux_nums specified for the imux group #%d.\n", i_g);
        }
        
        
        int num_froms = count_children(Node, "from", loc_data);
        imx_g.imux_from_inf.reserve(num_froms);
        SubElem = get_first_child(Node, "from", loc_data);
        for (int i_f = 0; i_f < num_froms; i_f++) {
            t_imux_from_inf imx_f;
            ProcessImuxFrom(SubElem, imx_f, loc_data);
            imx_g.imux_from_inf.push_back(imx_f);
            SubElem = SubElem.next_sibling(SubElem.name());
        }
        

        /* assign the imx_g to the imux_group_inf vector */
        imx.imux_group_inf.push_back(imx_g);

        Node = Node.next_sibling(Node.name());
    }

    //处理多级mux信息
    Node = get_single_child(IMux_Node, "multistage_muxs", loc_data, OPTIONAL);
    if (Node) {
        expect_only_children(Node, {"first_stage", "second_stage", "zero_stage"}, loc_data);
        SubElem = get_single_child(Node, "first_stage", loc_data);

        std::string first_stage_switch_name = get_attribute(SubElem, "switch_name", loc_data).as_string();
        imx.first_stage_switch_id = -1;
        for(int i = 0; i < arch->num_switches; i++){
            if(arch->Switches[i].name == first_stage_switch_name){
                imx.first_stage_switch_id = i;
                break;
            }
        }
        if(imx.first_stage_switch_id == -1){
            archfpga_throw(loc_data.filename_c_str(), loc_data.line(SubElem),
                           "Can't find first_stage switch %s for imux.\n", first_stage_switch_name.c_str());
        }
        ProcessFirstStage(SubElem, imx.first_stages, loc_data);
        SubElem = get_single_child(Node, "second_stage", loc_data);
        ProcessSecondStage(SubElem, imx.second_stages, loc_data);
        imx.twostage_mux_spec = true;
        SubElem = get_single_child(Node, "zero_stage", loc_data, OPTIONAL);
        if(SubElem){
            std::string zero_stage_switch_name = get_attribute(SubElem, "switch_name", loc_data).as_string();
            imx.zero_stage_switch_id = -1;
            for(int i = 0; i < arch->num_switches; i++){
                if(arch->Switches[i].name == zero_stage_switch_name){
                    imx.zero_stage_switch_id = i;
                    break;
                }
            }
            if(imx.zero_stage_switch_id == -1){
                archfpga_throw(loc_data.filename_c_str(), loc_data.line(SubElem),
                               "Can't find zero_stage switch %s for imux.\n", zero_stage_switch_name.c_str());
            }
            ProcessFirstStage(SubElem, imx.zero_stages, loc_data);
        }
    }
    else{
        imx.twostage_mux_spec = false;
    }
    
    arch->imux_inf.push_back(imx);
    return;
...
static void ProcessGsbFrom(pugi::xml_node From_Node, t_gsb_from_inf& gsb_f, const pugiutil::loc_data& loc_data) {
    ...
    itmp = get_attribute(From_Node, "only_detail", loc_data, OPTIONAL).as_int(0);
    gsb_f._only_detail = itmp ? true : false;

    itmp = get_attribute(From_Node, "use_group", loc_data, OPTIONAL).as_int(0);
    gsb_f._only_use_group = itmp ? true : false;

    expect_only_children(From_Node, {"detail", "group"}, loc_data);
    int num_details = count_children(From_Node, "detail", loc_data, OPTIONAL);
    gsb_f._gsb_detail_inf.clear();
    Node = get_first_child(From_Node, "detail", loc_data, OPTIONAL);
    for (int i_d = 0; i_d < num_details; i_d++) {
        std::vector<t_detail_from_inf> detail_froms;
        ProcessFromDetail(Node, detail_froms, loc_data);
        gsb_f._gsb_detail_inf.insert(gsb_f._gsb_detail_inf.end(), detail_froms.begin(), detail_froms.end());
        Node = Node.next_sibling(Node.name());
    }
    /*
    if(gsb_f.only_detail){
        tmp = get_attribute(From_Node, "type", loc_data).as_string();
        gsb_f.from_type = string_to_fromtype(tmp);
        tmp = get_attribute(From_Node, "name", loc_data).as_string();
        if(tmp){
            auto names = vtr::split(tmp," ");
            for (const auto& name : names) {
                gsb_f.from_name.push_back(name);
            }
        }
        return;
    }
    */
    ...
    if(tmp){
        std::string ss = tmp;
        gsb_f.to_fix_order.reserve(ss.length());
        //VTR_LOG("%d",ss.size());
        for (const char ch : ss) {
            //int ii=vtr::atoi(std::string(&ch));
            gsb_f.to_fix_order.push_back(vtr::atoi(std::string(1,ch)));
        }
    }*/

    int num_groups = count_children(From_Node, "group", loc_data, OPTIONAL);
    gsb_f._gsb_todir_group_inf.clear();
    Node = get_first_child(From_Node, "group", loc_data, OPTIONAL);
    for (int i_g = 0; i_g < num_groups; i_g++) {
        //std::vector<t_gsb_todir_group_inf> group_froms;
        t_gsb_todir_group_inf gf;
        tmp = get_attribute(Node, "to_dir", loc_data).as_string(nullptr);
        gf.to_dir = string_to_segdir(tmp);

        tmp = get_attribute(Node, "from_dir", loc_data, OPTIONAL).as_string(nullptr);
        if(tmp){
            auto fds = vtr::split(tmp," ");
            for (const auto& fd : fds) {
                gf.from_dir.push_back(string_to_segdir(fd));
            }
        }
        else{
            gf.from_dir.push_back(e_seg_dir::NUM_D_TYPES);
        }

        gf.actual_start = get_attribute(Node, "actual_start", loc_data, OPTIONAL).as_int(0);
        gf.actuall_foreach = get_attribute(Node, "actual_foreach", loc_data, OPTIONAL).as_int(0);
        ProcessFrom(Node, gf, loc_data, OPTIONAL);
        gsb_f._gsb_todir_group_inf.push_back(gf);
        Node = Node.next_sibling(Node.name());
    }

    /*
    if(gsb_f.only_use_group){
        tmp = get_attribute(From_Node, "type", loc_data).as_string();
        gsb_f.from_type = string_to_fromtype(tmp);
        tmp = get_attribute(From_Node, "name", loc_data).as_string();
        if(tmp){
    ...
    */
    ProcessFrom(From_Node, gsb_f, loc_data);
    return;
}
...
static void ProcessGSB(pugi::xml_node Gsb_Node, t_arch* arch, const pugiutil::loc_data& loc_data) {
    ...
    for (int i_g = 0; i_g < num_groups; i_g++) {
        ...
        int num_froms = count_children(Node, "from", loc_data);
        gsb_g.gsb_from_inf.reserve(num_froms);
        SubElem = get_first_child(Node, "from", loc_data);
        for (int i_f = 0; i_f < num_froms; i_f++) {
            t_gsb_from_inf gsb_f;
            ProcessGsbFrom(SubElem, gsb_f, loc_data);
            gsb_g.gsb_from_inf.push_back(gsb_f);
            SubElem = SubElem.next_sibling(SubElem.name());
            //VTR_LOG("detail_f.size=%d\n",gsb_g.gsb_from_inf[i_f].gsb_detail_inf.size());
        }
        /* assign the imx_g to the imux_group_inf vector */
    ...
    Node = get_single_child(Gsb_Node, "multistage_muxs", loc_data, OPTIONAL);
    if(Node) {
        expect_only_children(Node, {"multistage_mux", "first_stage", "second_stage", "zero_stage"}, loc_data);
        int num_multistage_muxs = count_children(Node, "multistage_mux", loc_data, OPTIONAL);
        if(num_multistage_muxs > 0){
            gsb.gsb_multistage_mux_inf.reserve(num_multistage_muxs);
            SubElem = get_first_child(Node, "multistage_mux", loc_data);
            for (int i_m = 0; i_m < num_multistage_muxs; i_m++) {
                t_gsb_multistage_mux_inf multistage_mux_inf;
                ProcessGsbMultistageMux(SubElem, multistage_mux_inf, loc_data);
                gsb.gsb_multistage_mux_inf.push_back(multistage_mux_inf);
                SubElem = SubElem.next_sibling(SubElem.name());
            }
        }

        //处理多级mux信息
        SubElem = get_single_child(Node, "first_stage", loc_data, OPTIONAL);
        if(SubElem){
            std::string first_stage_switch_name = get_attribute(SubElem, "switch_name", loc_data).as_string();
            gsb.first_stage_switch_id = -1;
            for(int i = 0; i < arch->num_switches; i++){
                if(arch->Switches[i].name == first_stage_switch_name){
                    gsb.first_stage_switch_id = i;
                    break;
                }
            }
            if(gsb.first_stage_switch_id == -1){
                archfpga_throw(loc_data.filename_c_str(), loc_data.line(SubElem),
                               "Can't find first_stage switch %s for gsb.\n", first_stage_switch_name.c_str());
            }
            ...
            if(SubElem){
                std::string zero_stage_switch_name = get_attribute(SubElem, "switch_name", loc_data).as_string();
                gsb.zero_stage_switch_id = -1;
                for(int i = 0; i < arch->num_switches; i++){
                    if(arch->Switches[i].name == zero_stage_switch_name){
                        gsb.zero_stage_switch_id = i;
                        break;
                    }
                }
                if(gsb.zero_stage_switch_id == -1){
                    archfpga_throw(loc_data.filename_c_str(), loc_data.line(SubElem),
                                   "Can't find zero_stage switch %s for gsb.\n", zero_stage_switch_name.c_str());
                }
                ProcessSecondStage(SubElem, gsb.zero_stages, loc_data);
            }
...
static void ProcessGSBarchs(pugi::xml_node Parent, t_arch* arch, const pugiutil::loc_data& loc_data) {
    ...
    expect_only_children(Parent, {"omux", "imux", "gsb"}, loc_data);

    //<omux> flag
    int num_omuxs = count_children(Parent, "omux", loc_data);
    arch->omux_inf.reserve(num_omuxs);
    Node = get_first_child(Parent, "omux", loc_data);
    for (int i_omux = 0; i_omux < num_omuxs; i_omux++) {
        /* use a temp variable which will be assigned to omux later */
        t_omux_inf omx;
        ...
        if (tmp) {
            ...
            archfpga_throw(loc_data.filename_c_str(), loc_data.line(Node),
                            "No name specified for the omux #%d.\n", i_omux);
        }

        tmp = get_attribute(Node, "pbtype_name", loc_data).as_string(nullptr);
        if (tmp) {
            omx.pbtype_names = vtr::split(tmp, " ");
        }
        else {
            archfpga_throw(loc_data.filename_c_str(), loc_data.line(Node),
                            "No pb_type specified for the omux #%d.\n", i_omux);
        }

        /* get mux_nums */
        itmp = get_attribute(Node, "mux_nums", loc_data).as_int(0);
        if (itmp) {
            omx.mux_nums = itmp;
        }
        else {
            archfpga_throw(loc_data.filename_c_str(), loc_data.line(Node),
                            "No mux_nums specified for the omux #%d.\n", i_omux);
        }
        
        tmp = get_attribute(Node, "arch_omux_switch", loc_data).as_string(nullptr);
        if (tmp) {
            /* Match names */
            int j;
            int NumSwitches = arch->num_switches;
            ...
            omx.arch_omux_switch = j;
        }
        else {
            archfpga_throw(loc_data.filename_c_str(), loc_data.line(Node),
                            "No arch_omux_switch specified for the omux #%d.\n", i_omux);
        }
        
        /* get the from infomation */
        SubElem = get_single_child(Node, "from", loc_data);
        omx.total_froms = get_attribute(SubElem, "total_froms", loc_data).as_int();
        omx.num_foreach = get_attribute(SubElem, "num_foreach", loc_data, OPTIONAL).as_int(1);
        omx.offset = get_attribute(SubElem, "offset", loc_data, OPTIONAL).as_int(0);

        //handle the multistage mux infos
        pugi::xml_node O_Node = get_single_child(Node, "multistage_muxs", loc_data, OPTIONAL);
        if (O_Node) {
            expect_only_children(O_Node, {"first_stage", "second_stage"}, loc_data);
            SubElem = get_single_child(O_Node, "first_stage", loc_data);

            std::string first_stage_switch_name = get_attribute(SubElem, "switch_name", loc_data).as_string();
            omx.first_stage_switch_id = -1;
            for(int i = 0; i < arch->num_switches; i++){
                if(arch->Switches[i].name == first_stage_switch_name){
                    omx.first_stage_switch_id = i;
                    break;
                }
            }
            if(omx.first_stage_switch_id == -1){
                archfpga_throw(loc_data.filename_c_str(), loc_data.line(SubElem),
                               "Can't find first_stage switch %s for omux.\n", first_stage_switch_name.c_str());
            }
            ProcessFirstStage(SubElem, omx.first_stages, loc_data);
            SubElem = get_single_child(O_Node, "second_stage", loc_data);
            ProcessSecondStage(SubElem, omx.second_stages, loc_data);
            omx.twostage_mux_spec = true;
        }
        else{
            omx.twostage_mux_spec = false;
        }

        /* assign the omux to the omux_inf vector */
        arch->omux_inf.push_back(omx);

        Node = Node.next_sibling(Node.name());
    }

    int num_imuxs = count_children(Parent, "imux", loc_data);
    arch->imux_inf.reserve(num_imuxs);
    Node = get_first_child(Parent, "imux", loc_data);
    for (int i_imux = 0; i_imux < num_imuxs; i_imux++) {
        ProcessIMUX(Node, arch, loc_data);
        Node = Node.next_sibling(Node.name());
    }
...
e_from_type string_to_fromtype(std::string fromtype_str) {
    ...
    } else if (fromtype_str == "omux") {
        fromtype = e_from_type::FT_OMUX;
    } else if (fromtype_str == "pb") {
        fromtype = e_from_type::FT_PB;
    } else if (fromtype_str == "imux") {
        fromtype = e_from_type::FT_IMUX;
    } else if (fromtype_str == "cas") {
        fromtype = e_from_type::FT_CAS;
    } else if (fromtype_str == "zero") {
        fromtype = e_from_type::FT_zero;
    } else {
        archfpga_throw(__FILE__, __LINE__,
