def inverter_generate(filename, use_finfet, use_technology):
    ...
    if not use_finfet:
        ...
        spice_file.write(
            "AS=Wn*trans_diffusion_length AD=Wn*trans_diffusion_length PS=Wn+2*trans_diffusion_length PD=Wn+2*trans_diffusion_length\n")
        spice_file.write("MPUP n_out n_in n_vdd n_vdd pmos L=gate_length W=Wp ")
        spice_file.write(
            "AS=Wp*trans_diffusion_length AD=Wp*trans_diffusion_length PS=Wp+2*trans_diffusion_length PD=Wp+2*trans_diffusion_length\n")
        ...
        spice_file.write(
            "ASEO=Wn*min_tran_width*trans_diffusion_length ADEO=Wn*min_tran_width*trans_diffusion_length PSEO=Wn*min_tran_width+2*trans_diffusion_length PDEO=Wn*min_tran_width+2*trans_diffusion_length\n")
        spice_file.write("MPUP n_out n_in n_vdd n_vdd pmos L=gate_length nfin=Wp ")
        spice_file.write(
            "ASEO=Wp*min_tran_width*trans_diffusion_length ADEO=Wp*min_tran_width*trans_diffusion_length PSEO=Wp*min_tran_width+2*trans_diffusion_length PDEO=Wp*min_tran_width+2*trans_diffusion_length\n")
        spice_file.write(".ENDS\n\n\n")
