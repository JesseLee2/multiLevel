...
def assign_firstStageMux_from(segs, seg_from, mux_nums, seg_from_assign, offset, assign_from, assignNumforeachOffset,
...
def assign_firstStageMux_pin_from(segs, pin_froms, mux_nums, seg_from_assign, offset_first, assign_from, assignNumforeachOffset_first,
                                to_track_to_first_mux, StageMuxFroms, length_reuse_threshold, first_mux_assign, segs_times):
    if len(pin_froms) > 1:
        for i in range(mux_nums):
            ...
            if (type_str == 'pb'):
                ...
                from_detail = "GSB_zero" + str(assign_from["GSB_zero"])
                assign_from["GSB_zero"] = (assign_from["GSB_zero"] + 1) % 8
                stagemuxfrom = TwoStageMuxFrom_inf(type_str, name_str, from_detail)
                if mux_name in StageMuxFroms:
                    StageMuxFroms[mux_name].append(stagemuxfrom)
                else:
