...
def objective(variables, space, archTree, base_result, new_arch_name, exec_file, blif_dir, blif_names, connect_string, same_trial_limit):
    ...
    print('pass prune 2')
    # if chanWidth < 80:  
    if chanWidth < 130 or chanWidth > 240: 
        return {
            'why': 'chanWidth',
            'status': STATUS_FAIL,
        }
    ...
    for blif_name in blif_names:
        if pd.isna( base_result['Crit Path'][blif_name] ):
            ...
            subprocess.run([exec_file, '../../'+new_arch_name+'.xml', blif_dir+'/'+blif_name+'.blif',  '--route_chan_width', str(chanWidth),  '--timing_report_detail', 'aggregated', '--max_router_iterations', '70'], cwd = new_arch_name+'/'+blif_name, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
