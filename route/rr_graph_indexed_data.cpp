...
static void load_rr_indexed_data_T_values(int index_start,
    ...
    for (itrack = 0; itrack < nodes_per_chan + medium_num; itrack++) {
        if(itrack < nodes_per_chan){
            ...
            if (medium_index < medium_seg_inf.imux_mdeium_num_xory) {
                medium_index = is_y_medium ? medium_index + medium_seg_inf.imux_mdeium_num_xory : medium_index;
            }
            else if (medium_index >= medium_seg_inf.imux_mdeium_num_xory && medium_index < medium_seg_inf.imux_mdeium_num_xory + medium_seg_inf.omux_mdeium_num_xory){
                medium_index = medium_index - medium_seg_inf.imux_mdeium_num_xory;
                medium_index = is_y_medium ? medium_index + medium_seg_inf.omux_mdeium_num_xory : medium_index;
                medium_type = 1;//omux
            }
            else if (medium_index >= medium_seg_inf.imux_mdeium_num_xory + medium_seg_inf.omux_mdeium_num_xory) {
                medium_index = medium_index - medium_seg_inf.imux_mdeium_num_xory - medium_seg_inf.omux_mdeium_num_xory;
