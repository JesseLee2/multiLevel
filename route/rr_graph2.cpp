...
int count_imux_medium_num(const t_imux_inf& imx, std::map<int, int>& clbpin_medium_map);
int count_gsb_medium_num(const t_gsb_inf& gsb);

int count_imux_medium_num_v2(const t_imux_inf& imx, std::map<int, int>& clbpin_medium_map, std::map<std::string, int>& mux_name_medium_map);
int count_gsb_medium_num_v2(const t_gsb_inf& gsb, std::map<std::string, int>& mux_name_medium_map);
int count_omux_medium_num_v2(const t_omux_inf& gsb, std::map<std::string, int>& mux_name_medium_map);
...
t_seg_details* alloc_and_load_seg_details(int* max_chan_width,
    ...
    bool is_omux_medium, is_imux_medium, is_gsb_medium;
    ...
    *max_chan_width = tmp;
    ...
    mux_name_medium_map_vec.resize(3);
    int imux_medium_num, gsb_medium_num;

    if (arch->imux_inf[0].twostage_mux_spec) {
        imux_medium_num = count_imux_medium_num_v2(arch->imux_inf[0], clbpin_medium_map, mux_name_medium_map_vec[0]); //指定了所有多级复用mux
        //count the number of the first and second level muxes
    } else {
        imux_medium_num = count_imux_medium_num(arch->imux_inf[0], clbpin_medium_map); //目前只考虑一种gsb，即所有都一样，因此只有一种imux，其他同理。
    }

    if (arch->gsb_inf[0].twostage_mux_spec) {
        gsb_medium_num = count_gsb_medium_num_v2(arch->gsb_inf[0], mux_name_medium_map_vec[1]); //指定了所有多级复用mux
        //only count the number of the first level muxes
    } else {
        gsb_medium_num = count_gsb_medium_num(arch->gsb_inf[0]); //目前只考虑一种gsb，即所有都一样，因此只有一种imux，其他同理。
    }

    int omux_medium_num;
    if (arch->omux_inf[0].twostage_mux_spec) {
        omux_medium_num = count_omux_medium_num_v2(arch->omux_inf[0], mux_name_medium_map_vec[2]); //指定了所有多级复用mux
        //count the number of the first and second level muxes
    } else {
        omux_medium_num = arch->omux_inf[0].mux_nums; //目前只考虑一种gsb，即所有都一样，因此只有一种imux，其他同理。
    }



    //assign medium node
    std::vector<int> medium_num_xory={int(ceil(imux_medium_num/2.0)), int(ceil(omux_medium_num/2.0)), int(ceil(gsb_medium_num/2.0))};
    //std::vector<int> medium_num_xory={imux_medium_num, omux_medium_num, gsb_medium_num};
    std::vector<std::string> medium_seg_names={"imux_medium", "omux_medium", "gsb_medium"};
    std::vector<int> arch_switch = {segment_inf[segment_inf.size() - 3].arch_wire_switch, segment_inf[segment_inf.size() - 2].arch_wire_switch, segment_inf[segment_inf.size()-1].arch_wire_switch};

    //gsb内的mux默认只有一种，因此只取第一种线型的mux
    medium_seg_inf.imux_mdeium_num_xory = medium_num_xory[0];
    medium_seg_inf.omux_mdeium_num_xory = medium_num_xory[1];
    medium_seg_inf.gsb_mdeium_num_xory = medium_num_xory[2];
    medium_seg_inf.imux_medium_num = imux_medium_num;
    medium_seg_inf.omux_medium_num = omux_medium_num;
    medium_seg_inf.gsb_medium_num = gsb_medium_num;

    int total_seg_details = *max_chan_width + medium_num_xory[0] +medium_num_xory[1] + medium_num_xory[2];
    //int total_seg_details = *max_chan_width + ceil(imux_medium_num /2.0) + ceil(omux_medium_num/2.0) + ceil(gsb_medium_num/2.0);
    ...
    for (size_t i = 0; i < segment_inf.size(); ++i) {
        ...
        is_omux_medium = segment_inf[i].is_omux_medium;
        is_imux_medium = segment_inf[i].is_imux_medium;
        ...
        for (size_t ipart = 0; ipart < tmp_seg_len.size(); ipart++) {
            for (itrack = 0; itrack < (tmp_seg_len[ipart] * ntracks) / length; itrack++) {
                ...
                seg_details[cur_track].is_omux_medium = is_omux_medium;
                seg_details[cur_track].is_imux_medium = is_imux_medium;
    ...
    for(int i = 0; i < 3; i++){
        ...
        arch_wire_switch = arch_switch[i];//??????
        arch_opin_switch = arch_switch[i];
        VTR_ASSERT((arch_wire_switch == arch_opin_switch) || (directionality != UNI_DIRECTIONAL));

        is_imux_medium = i==0? true:false;
        is_omux_medium = i==1? true:false;
        is_gsb_medium = i==2? true:false;

        medium_seg_name=medium_seg_names[i];
        ...
        for (itrack = 0; itrack < ntracks; itrack++) {
            ...
            seg_details[cur_track].index = segment_inf.size() -3 + i;////???????????????
            seg_details[cur_track].cost_index = cost_index_num + i;
            VTR_ASSERT(0 == strcmp(segment_inf[segment_inf.size() -3 + i].name.c_str(), medium_seg_name.c_str()));
...
t_chan_details init_chan_details(const DeviceGrid& grid,
    ...
    int medium_num_xory = medium_seg_inf.imux_mdeium_num_xory + medium_seg_inf.omux_mdeium_num_xory + medium_seg_inf.gsb_mdeium_num_xory;
    ...
    for (size_t x = 0; x < grid.width(); ++x) {
        for (size_t y = 0; y < grid.height(); ++y) {
            ...
            for (int i = 0; i < num_seg_details; ++i) {
                ...
                /*if((x == 0 || x == grid.width() - 1 || y == 0 || y == grid.height() - 1) && grid[x][y].type == device_ctx.EMPTY_TYPE) {
                ...
                if (seg_details_type == SEG_DETAILS_X) {//可能有问题
...
void obstruct_chan_details(const DeviceGrid& grid,
                           ...
                           t_chan_details& chan_details_y) {
    auto& device_ctx = g_vpr_ctx.device();
    int track_max = nodes_per_chan->max + medium_seg_inf.imux_mdeium_num_xory + medium_seg_inf.omux_mdeium_num_xory + medium_seg_inf.gsb_mdeium_num_xory;
    ...
    for (int chan = 0; chan < num_chans - 1; ++chan) {
        for (int seg = 1; seg < chan_len - 1; ++seg) {
            ...
            for (unsigned track = 0; track < indices[type][chan][seg][0].size(); ++track) {
                ...
                if(seg_details[track].is_imux_medium() || seg_details[track].is_omux_medium() || seg_details[track].is_gsb_medium())
                    continue;
...
void load_sblock_bend_pattern_lookup_for_gsb(const int i,
    ...
    for (e_side side : {TOP, RIGHT, BOTTOM, LEFT}) {
        ...
        //例如，第1,3,5是incoming_wire,则有incoming_wire_label[]={UN_SET, 0, UN_SET, 1, UN_SET, 2, ...}
        ...
        //与上面不同，例如，第1,3,5是bent_middle_wire,则有incoming_bend_end_label[]={1,3,5}
...
int count_imux_medium_num(const t_imux_inf& imx, std::map<int, int>& clbpin_medium_map){
    ...
    //int num_lut_eachgroup=0;

    clbpin_medium_map.clear();

    for (i = 0; i < device_ctx.num_block_types; i++) {
        if (strcmp(device_ctx.block_types[i].name, imx.pbtype_names[0].c_str()) == 0) {//第一个是clb，其余IG和它一样，因此以clb的ig来计算
            break;
        }
    }
    if (i >= device_ctx.num_block_types) {
        vpr_throw(VPR_ERROR_ARCH, get_arch_file_name(), 1, "Unable to find block %s for imux.\n", imx.pbtype_names[0].c_str());
    }

    auto const& clb_type = device_ctx.block_types[i];
    auto const pb_type = clb_type.pb_type;
    
    for(i = 0; i < imx.lut_group ; i++){
        //num_lut_eachgroup = imx.imux_group_inf[i].lut_name.size();
        for(auto const& imx_from : imx.imux_group_inf[i].imux_from_inf){
            if(imx_from.from_type != e_from_type::FT_IMUX) continue;
            //int group_total_froms=0;
            for(auto const& port_name : imx_from.pin_types){
                auto it=find(collected_ports.begin(), collected_ports.end(), port_name);
                if(it != collected_ports.end()) continue;
                ...
                if (j >= pb_type->num_ports) {
                    vpr_throw(VPR_ERROR_ARCH, get_arch_file_name(), 1, "Unable to find port %s (on block %s).\n", port_name.c_str(), imx.pbtype_names[0].c_str());
                }
                //group_total_froms += pb_type->ports[j].num_pins;

                for(k = 0; k < pb_type->ports[j].num_pins; k++){
                    int pin_index;
                    get_blk_pin_from_port_pin(clb_type.index, j, k, &pin_index);
                    clbpin_medium_map[pin_index] = k + totals;
                }

                collected_ports.push_back(port_name);
                totals += pb_type->ports[j].num_pins;
            }
            //VTR_ASSERT(imx_from.total_froms == totals);
        }
    }
    return totals;
}
...
int count_imux_medium_num_v2(const t_imux_inf& imx, std::map<int, int>& clbpin_medium_map, std::map<std::string, int>& mux_name_medium_map) {
    ...
    int first_stage_num = imx.first_stages.size();
    for (i = 0; i < first_stage_num; i++) {
        mux_name_medium_map[imx.first_stages[i].mux_name] = i + totals;
    }
    totals += first_stage_num;

    int zero_stage_num = imx.zero_stages.size();
    for (i = 0; i < zero_stage_num; i++) {
        mux_name_medium_map[imx.zero_stages[i].mux_name] = i + totals;
    }
    totals += zero_stage_num;
    return totals;
}
...
int count_gsb_medium_num(const t_gsb_inf& gsb){
    ...
    for(auto const& gm : gsb.gsb_multistage_mux_inf){
        totals += gm.normal_mux_infs.size()+1;  
    }
    return totals;
}
...
int count_gsb_medium_num_v2(const t_gsb_inf& gsb, std::map<std::string, int>& mux_name_medium_map) {
    ...
    //gsb_medium_id_vec.resize(first_stage_num);
    for (i = 0; i < first_stage_num; i++) {
        mux_name_medium_map[gsb.first_stages[i].mux_name] = i;
        //gsb_medium_id_vec[i] = gsb.first_stages[i].medium_id;
    }
    totals += first_stage_num;

    int zero_stage_num = gsb.zero_stages.size();
    //gsb_medium_id_vec.resize(first_stage_num);
    for (i = 0; i < zero_stage_num; i++) {
        mux_name_medium_map[gsb.zero_stages[i].mux_name] = i + totals;
        //gsb_medium_id_vec[i] = gsb.first_stages[i].medium_id;
    }
    totals += zero_stage_num;
...
int count_omux_medium_num_v2(const t_omux_inf& omx, std::map<std::string, int>& mux_name_medium_map) {
    auto& device_ctx = g_vpr_ctx.device();
    int i;
    int totals = 0;

    for (i = 0; i < device_ctx.num_block_types; i++) {
        if (strcmp(device_ctx.block_types[i].name, omx.pbtype_names[0].c_str()) == 0) {
            break;
        }
    }
    if (i >= device_ctx.num_block_types) {
        vpr_throw(VPR_ERROR_ARCH, get_arch_file_name(), 1, "Unable to find block %s for imux.\n", omx.pbtype_names[0].c_str());
    }

    int first_stage_num = omx.first_stages.size();
    //gsb_medium_id_vec.resize(first_stage_num);
    for (i = 0; i < first_stage_num; i++) {
        mux_name_medium_map[omx.first_stages[i].mux_name] = i;
        //gsb_medium_id_vec[i] = gsb.first_stages[i].medium_id;
    }
    totals += first_stage_num;

    int second_stage_num = omx.second_stages.size();
    //gsb_medium_id_vec.resize(first_stage_num);
    for (i = 0; i < second_stage_num; i++) {
        mux_name_medium_map[omx.second_stages[i].mux_name] = i + totals;
        //gsb_medium_id_vec[i] = gsb.first_stages[i].medium_id;
    }
    totals += second_stage_num;
...
void load_imux_medium_rr_indices(const DeviceGrid& grid,
                                  ...
                                  const t_chan_details& chan_details_y,
                                  int* index){
    int medium_num_xory = medium_seg_inf.gsb_mdeium_num_xory + medium_seg_inf.imux_mdeium_num_xory + medium_seg_inf.omux_mdeium_num_xory;
    auto const& L_rr_node_indices = g_vpr_ctx.mutable_device().rr_node_indices;
    for(int imux_types_index = 0; imux_types_index < arch->imux_inf.size(); imux_types_index++){
        auto const& imx = arch->imux_inf[imux_types_index];
        VTR_ASSERT(indices.size() == size_t(grid.width()));
        std::map<int, int> no_use;
        int imux_mdeium_num = medium_seg_inf.imux_medium_num;
        for (int x = 0; x < grid.width()-1; x++) {
            VTR_ASSERT(indices[x].size() == size_t(grid.height()));
            for (int y = 0; y < grid.height()-1; y++) {
                VTR_ASSERT(indices[x][y].size() == size_t(3));
                VTR_ASSERT(indices[x][y][0].size() == size_t(arch->imux_inf.size()));
                //if (grid[x][y].width_offset == 0 && grid[x][y].height_offset == 0) {//大块如何处理
                    t_type_ptr type = grid[x][y].type;
                    bool is_gsb_arch = false;
                    for(auto pb_name : imx.pbtype_names){
                    ...
                    if (is_gsb_arch) {
                        int i_imux = 0;
                        indices[x][y][0][imux_types_index].resize(imux_mdeium_num, OPEN);
                        ...
                        for (unsigned track = L_rr_node_indices[CHANX][y][x][0].size(); track < L_rr_node_indices[CHANX][y][x][0].size() + medium_num_xory; ++track) {
                            if (seg_details_x[track].is_imux_medium()) {
                                VTR_ASSERT(seg_details_x[track].length() == 1);
                                if (i_imux < imux_mdeium_num) {
                                    int inode = *index;
                                    ++(*index);
                                    indices[x][y][0][imux_types_index][i_imux] = inode;
                                    i_imux++;
                                } else {
                                    break;
                                }
                        ...
                        for (unsigned track = L_rr_node_indices[CHANY][x][y][0].size(); track < L_rr_node_indices[CHANY][x][y][0].size() + medium_num_xory; ++track) {
                            if (seg_details_y[track].is_imux_medium()) {
                                VTR_ASSERT(seg_details_x[track].length() == 1);
                                if (i_imux < imux_mdeium_num) {
                                    int inode = *index;
                                    ++(*index);
...
void load_omux_medium_rr_indices(const DeviceGrid& grid,
    ...
    int medium_num_xory = medium_seg_inf.gsb_mdeium_num_xory + medium_seg_inf.imux_mdeium_num_xory + medium_seg_inf.omux_mdeium_num_xory;
    auto const& L_rr_node_indices = g_vpr_ctx.mutable_device().rr_node_indices;
    for(int omux_types_index = 0; omux_types_index < arch->omux_inf.size(); omux_types_index++){
        auto const& omx = arch->omux_inf[omux_types_index];
        VTR_ASSERT(indices.size() == size_t(grid.width()));
        int omux_mdeium_num = medium_seg_inf.omux_medium_num;
        for (int x = 0; x < grid.width()-1; x++) {
            VTR_ASSERT(indices[x].size() == size_t(grid.height()));
            for (int y = 0; y < grid.height()-1; y++) {
                VTR_ASSERT(indices[x][y].size() == size_t(3));
                VTR_ASSERT(indices[x][y][1].size() == size_t(arch->omux_inf.size()));
                //if (grid[x][y].width_offset == 0 && grid[x][y].height_offset == 0) {//大块如何处理
                    t_type_ptr type = grid[x][y].type;
                    bool is_gsb_arch = false;
                    for (auto pb_name : omx.pbtype_names) {
                        if (0 == strcmp(type->name, pb_name.c_str())) {
                            is_gsb_arch = true;
                            break;
                        }
                    ...
                    if (is_gsb_arch) {
                        int i_omux = 0;
                        indices[x][y][1][omux_types_index].resize(omux_mdeium_num, OPEN);
                        const t_chan_seg_details* seg_details_x = chan_details_x[x][y].data();
                        for (unsigned track = L_rr_node_indices[CHANX][y][x][0].size(); track < L_rr_node_indices[CHANX][y][x][0].size() + medium_num_xory; ++track) {
                            if (seg_details_x[track].is_omux_medium()) {
                                VTR_ASSERT(seg_details_x[track].length() == 1);
                                if (i_omux < omux_mdeium_num) {
                                    int inode = *index;
                                    ++(*index);
                                    indices[x][y][1][omux_types_index][i_omux] = inode;
                                    i_omux++;
                                } else {
                                    break;
                                }
                        ...
                        const t_chan_seg_details* seg_details_y = chan_details_y[x][y].data();
                        for (unsigned track = L_rr_node_indices[CHANY][x][y][0].size(); track < L_rr_node_indices[CHANY][x][y][0].size() + medium_num_xory; ++track) {
                            if (seg_details_y[track].is_omux_medium()) {
                                VTR_ASSERT(seg_details_x[track].length() == 1);
                                if (i_omux < omux_mdeium_num) {
                                    int inode = *index;
                                    ++(*index);
...
void load_gsb_medium_rr_indices(const DeviceGrid& grid,
    ...
    for(int gsb_types_index = 0; gsb_types_index < arch->gsb_inf.size(); gsb_types_index++){
        ...
        for (int x = 0; x < grid.width()-1; x++) {
            ...
            for (int y = 0; y < grid.height()-1; y++) {
                VTR_ASSERT(indices[x][y].size() == size_t(3));
                VTR_ASSERT(indices[x][y][2].size() == size_t(arch->gsb_inf.size()));
                //if (grid[x][y].width_offset == 0 && grid[x][y].height_offset == 0) {//大块如何处理
                    t_type_ptr type = grid[x][y].type;
                    bool is_gsb_arch = false;
                    ...
                    if (is_gsb_arch) {
                        int i_gsb = 0;
                        indices[x][y][2][gsb_types_index].resize(gsb_mdeium_num, OPEN);
                        const t_chan_seg_details* seg_details_x = chan_details_x[x][y].data();
                        for (unsigned track = L_rr_node_indices[CHANX][y][x][0].size(); track < L_rr_node_indices[CHANX][y][x][0].size() + medium_num_xory; ++track) {
                            if (seg_details_x[track].is_gsb_medium()) {
                                ...
                                if (i_gsb < gsb_mdeium_num) {
                                    int inode = *index;
                                    ++(*index);
                                    indices[x][y][2][gsb_types_index][i_gsb] = inode; //每组第一个是reuse_mux，normal按顺序排
                                    i_gsb++;
                                } else {
                                    break;
                                }
                        ...
                        for (unsigned track = L_rr_node_indices[CHANY][x][y][0].size(); track < L_rr_node_indices[CHANY][x][y][0].size() + medium_num_xory; ++track) {
                            if (seg_details_y[track].is_gsb_medium()) {
                                ...
                                if (i_gsb < gsb_mdeium_num) {
                                    int inode = *index;
                                    ++(*index);
                                    indices[x][y][2][gsb_types_index][i_gsb] = inode;
                                    i_gsb++;
                                } else {
                                    break;
                                }
...
t_medium_rr_node_indices alloc_and_load_medium_rr_node_indices(const DeviceGrid& grid,
    ...
    for (size_t x = 0; x < grid.width(); ++x) {
        ...
        for (size_t y = 0; y < grid.height(); ++y) {
            indices[x][y].resize(3);//imux\omux\gsb(0/1)
            indices[x][y][0].resize(arch->imux_inf.size());
            indices[x][y][1].resize(arch->omux_inf.size());
            indices[x][y][2].resize(arch->gsb_inf.size());
        }
    }

    load_imux_medium_rr_indices(grid, arch, medium_seg_inf, indices, chan_details_x, chan_details_y, index);

    load_omux_medium_rr_indices(grid, arch, medium_seg_inf, indices, chan_details_x, chan_details_y, index);
...
int get_omuxOrpb_to_track_for_gsb(int x_coord, int y_coord, const DeviceGrid& grid,
    ...
    VTR_ASSERT(from_type == e_from_type::FT_OMUX || from_type == e_from_type::FT_PB);
    int edge_count = 0;
    int to_x, to_y;                         /* index into destination channel */
...
/*
    VTR_LOG("map_size = %d\n", other_pbpin_to_clbpin_map.size());
    for(auto key : other_pbpin_to_clbpin_map){
        VTR_LOG("key_str = %s\n", key.first.c_str());
    ...
    for (e_side from_side : {NUM_SIDES}) {
        for (e_side to_side : {TOP, RIGHT, BOTTOM, LEFT}) {
            GSB_Lookup gsb_conn(x_coord, y_coord, from_type, from_side, to_side); /* for indexing into FPGA's switchblock map */
            if (gsb_conn_map->count(gsb_conn) > 0){
                std::vector<t_gsb_edge>& conn_vector = (*gsb_conn_map)[gsb_conn];
                ...
                /* go through the connections... */
                for (int iconn = 0; iconn < (int)conn_vector.size(); ++iconn) {
                    int from_omuxOrpb_pin = conn_vector.at(iconn).from_wireOrpin;
                    int to_wire = conn_vector.at(iconn).to_wireOrpin;
                    int from_node;
                    ...
                    if( from_type == e_from_type::FT_OMUX){
                        from_node = get_medium_rr_node_index(L_medium_rr_node_indices, x_coord, y_coord, 1, 0, from_omuxOrpb_pin);//0-IMUX, 1-OMUX, 2-GSB。默认只有一种omux
                    }
                    else{
                        if(is_other_tile){
                            ...
                            auto iter2 = pbpin_to_clbpin_map_tile.find(from_omuxOrpb_pin);
                            if(iter2 != pbpin_to_clbpin_map_tile.end()){
                                from_omuxOrpb_pin = iter2->second;
                            }
                            else{
                                continue;
                            }
                        ...
                        for(auto side : SIDES){
                            from_node = get_rr_node_index(L_rr_node_indices, x_coord, y_coord, OPIN, from_omuxOrpb_pin, side);
                            if(from_node != OPEN)
                                break;
                        }
                        //from_node = get_rr_node_index(L_rr_node_indices, x_coord, y_coord, OPIN, from_omuxOrpb_pin);
                    }
...
int build_multistage_mux_for_gsb(int x_coord, int y_coord,
                                    t_rr_edge_info_set& rr_edges_to_create,
                                    t_gsb_multistage_mux_map* gsb_multistage_mux_map){
    int edge_count = 0;
    ...
    auto it = (*gsb_multistage_mux_map).find(gsb_ms);
    if (it != (*gsb_multistage_mux_map).end()) {
        multistage_mux_inf_gsbloc = (*it).second;
    }

    for(auto const& multistage_mux_map_inf : multistage_mux_inf_gsbloc.multistage_mux_map_infs){
        int to_node = 0;
        int switch_id;
        auto const& reuse_mux = multistage_mux_map_inf.reuse_medium_node;

        //to_mux conn 第二级
        for (int i = 0; i < multistage_mux_map_inf.to_nodes.size(); i++){
            //normal mux连到该去向
            to_node = multistage_mux_map_inf.to_nodes[i];
            if(to_node == 0)//to_node == 0 if there is no to_wire in bounds of array
                continue;
            switch_id = multistage_mux_map_inf.to_switches[i];
            auto it = multistage_mux_inf_gsbloc.to_node_loc_map.find(to_node);
            VTR_ASSERT(it != multistage_mux_inf_gsbloc.to_node_loc_map.end());
            auto const& from_node_inds = (*it).second.second;
            int from_node;
            for(int from_node_ind : from_node_inds){
                from_node = multistage_mux_map_inf.from_normal_muxes[from_node_ind].medium_node;
                rr_edges_to_create.emplace_back(from_node, to_node, switch_id);
                ++edge_count;

                //normal mux conn
                int to_medium_node = from_node;
                switch_id = multistage_mux_map_inf.to_switches[0];
                for (int from_node_first : multistage_mux_map_inf.from_normal_muxes[from_node_ind].medium_from_nodes) {
                    rr_edges_to_create.emplace_back(from_node_first, to_medium_node, switch_id);
                    ++edge_count;
                }
            }

            //reuse mux连到该去向
            from_node = reuse_mux.medium_node;
            rr_edges_to_create.emplace_back(from_node, to_node, switch_id);
            ++edge_count;
        }
        ...
        //reuse mux conn
        if(to_node != 0) {
            to_node = reuse_mux.medium_node;
            switch_id = multistage_mux_map_inf.to_switches[0];
            for (int from_node : reuse_mux.medium_from_nodes) {
                rr_edges_to_create.emplace_back(from_node, to_node, switch_id);
                ++edge_count;
            }
        }
        ...
        //normal mux conn
        /*
        for(auto const& normal_mux : multistage_mux_map_inf.from_normal_muxes){
            to_node = normal_mux.medium_node;
            switch_id = multistage_mux_map_inf.to_switches[0];
            for (int from_node : normal_mux.medium_from_nodes) {
                rr_edges_to_create.emplace_back(from_node, to_node, switch_id);
                ++edge_count;
            }
        }
...
int get_omuxOrpb_to_imux_for_imux(int x_coord, int y_coord,
    ...
    VTR_ASSERT(from_type == e_from_type::FT_OMUX || from_type == e_from_type::FT_PB || from_type == e_from_type::FT_IMUX);
    int edge_count = 0;

    if( 0 != strcmp(pbtype_name.c_str(), actual_pbtype_name.c_str()) && !is_other_tile)
        return edge_count;

    auto it = record_imux_coord.find(actual_pbtype_name);//记录的是该imux对应的第一种pbtype（clb）的位置
    if(it == record_imux_coord.end()) return edge_count;

    int record_x = it->second.first;
    int record_y = it->second.second;

    for (e_side from_side : {NUM_SIDES}) {
        for (e_side to_side : {NUM_SIDES}) {
            GSB_Lookup gsb_conn(record_x, record_y, from_type, from_side, to_side); /* for indexing into FPGA's switchblock map */
            if (imux_conn_map->count(gsb_conn) > 0){
                std::vector<t_gsb_edge>& conn_vector = (*imux_conn_map)[gsb_conn];
                ...
                for (int iconn = 0; iconn < (int)conn_vector.size(); ++iconn) {
                    ...
                    if( from_type == e_from_type::FT_OMUX){
                    ...
                    else if( from_type == e_from_type::FT_PB){
                        if (is_other_tile) {
                            int index = grid[x_coord][y_coord].width_offset * type->height + grid[x_coord][y_coord].height_offset;
                            auto const pbpin_to_clbpin_map_tile = (iter->second)[index];
                            auto iter2 = pbpin_to_clbpin_map_tile.find(from_omuxOrpb_pin);
                        ...
                        auto it = clbpin_to_medium.find(from_omuxOrpb_pin);
                        if(it != clbpin_to_medium.end()){
                            from_node = get_medium_rr_node_index(L_medium_rr_node_indices, x_coord, y_coord, 0, 0, it->second);//0-IMUX, 1-OMUX, 2-GSB
                        }
                        else {
                            VPR_THROW(VPR_ERROR_ROUTE, "can't find medium node for ipin %d as src.\n", from_omuxOrpb_pin);
                            /*
                            for(auto side : SIDES){
                                from_node = get_rr_node_index(L_rr_node_indices, x_coord, y_coord, IPIN, from_omuxOrpb_pin, side);
                                if(from_node != OPEN)
                                    break;
                            }*/
                            //from_node = get_rr_node_index(L_rr_node_indices, x_coord, y_coord, IPIN, from_omuxOrpb_pin);
                        }
                    }
                    ...
                    //int to_node = get_rr_node_index(L_rr_node_indices, to_x, to_y, to_chan_type, to_pb_pin);
                    if (from_node == OPEN || to_node == OPEN) {
                        continue;
                    }
...
int get_medium_to_pin_for_imux(int x_coord, int y_coord,
    ...
    auto it2 = record_imux_coord.find(actual_pbtype_name);//记录的是该imux对应的第一种pbtype（clb）的位置
...
int get_pb_to_omux(int x_coord, int y_coord,
    ...
    int edge_count = 0;
    if( 0 != strcmp(actual_pbtype_name.c_str(), type->name) && !is_other_tile) return edge_count;
    auto it = (*omux_conn_map).find(actual_pbtype_name);
    if(it == (*omux_conn_map).end()) return edge_count;

    const std::vector<std::vector<t_omux_edge>>& conn_vectors = (*omux_conn_map)[actual_pbtype_name];
    
    /* go through the connections... */
    for(int i_omux_pin = 0; i_omux_pin < conn_vectors.size(); i_omux_pin++){
        const std::vector<t_omux_edge>& conn_vector = conn_vectors[i_omux_pin];
        for (int iconn = 0; iconn < (int)conn_vector.size(); ++iconn) {
            int from_pb_pin = conn_vector.at(iconn).from_pb_pin;
            int to_omux_output = conn_vector.at(iconn).to_omux_output;

            int from_node;
            if (is_other_tile) {
                int index = grid[x_coord][y_coord].width_offset * type->height + grid[x_coord][y_coord].height_offset;
                auto const pbpin_to_clbpin_map_tile = (iter->second)[index];
                auto iter2 = pbpin_to_clbpin_map_tile.find(from_pb_pin);
                if (iter2 != pbpin_to_clbpin_map_tile.end()) {
                    from_pb_pin = iter2->second;
                } else {
                    continue;
                }
            }
            ...
            for(auto side : SIDES){
                from_node = get_rr_node_index(L_rr_node_indices, x_coord, y_coord, OPIN, from_pb_pin, side);
                if(from_node != OPEN)
                    break;
            }
            int to_node = get_medium_rr_node_index(L_medium_rr_node_indices, x_coord, y_coord, 1, 0, to_omux_output);//0-IMUX, 1-OMUX, 2-GSB
