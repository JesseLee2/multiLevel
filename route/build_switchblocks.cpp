...
static void compute_two_stage_omux_map(int x_coord, int y_coord,
                                      const t_chan_details& chan_details_x, const t_chan_details& chan_details_y,
                                      t_omux_inf* omux,
                                      const DeviceGrid& grid, t_wire_type_sizes* wire_type_sizes,
                                      t_two_stage_mux_map* two_stage_mux_map,
...
static std::vector<int> get_from_node_for_multistage_mux_v2(int x_coord, int y_coord, const t_chan_details& chan_details_x, const t_chan_details& chan_details_y, 
                                                         ...
                                                         const t_potential_wires_map& potential_wires_map, const std::map<int, int>& clbpin_medium_map, const std::map<std::string, int>& imux_stage_mux_medium_map, const std::map<std::string, int>& gsb_stage_mux_medium_map, const std::map<std::string, int>& omux_stage_mux_medium_map);
static std::vector<int> get_from_node_for_multistage_omux_v2(int x_coord, int y_coord, const t_chan_details& chan_details_x, const t_chan_details& chan_details_y,
                                                         const DeviceGrid& grid, t_wire_type_sizes* wire_type_sizes, std::vector<t_mux_same_from_inf> from_mux_inf,
                                                         const std::string first_pb_name, const std::map<std::string, std::vector<std::map<int, int>>> other_pbpin_to_clbpin_map,
                                                         const t_potential_wires_map& potential_wires_map, const std::map<int, int>& clbpin_medium_map, const std::map<std::string, int>& omux_stage_mux_medium_map);
    ...
    for (auto const& same_from : from_mux_inf) {
        if (same_from.mux_from_type == e_from_type::FT_SEGMENT) {
            ...
            for (int j = 0; j < same_from.from_inds.size(); j++) {
                ...
                int from_node = get_medium_rr_node_index(device_ctx.medium_rr_node_indices, x_coord, y_coord, 1, 0, omux_index); //0-IMUX, 1-OMUX, 2-GSB
                if(from_node == OPEN)
...
static void compute_two_stage_mux_map(int x_coord, int y_coord, 
    ...
    auto const& fsms = is_imux ? imux->first_stages : gsb->first_stages;
    auto const& ssms = is_imux ? imux->second_stages : gsb->second_stages;
    two_stage_mux_inf.first_stages.reserve(fsms.size());
    two_stage_mux_inf.second_stages.reserve(ssms.size());

    int switch_id = is_imux ? imux->first_stage_switch_id : gsb->first_stage_switch_id;
    int medium_type = is_imux ? 0 : 2;
    auto stage_mux_medium_map = is_imux ? imux_stage_mux_medium_map : gsb_stage_mux_medium_map;
    //first stage
    int medium_ind;
    for(auto const& fsm : fsms){
        ...
        sm.from_nodes = get_from_node_for_multistage_mux_v2(x_coord, y_coord, chan_details_x, chan_details_y,
                                                         ...
                                                         potential_wires_map, clbpin_medium_map, imux_stage_mux_medium_map, gsb_stage_mux_medium_map, omux_stage_mux_medium_map);
        sm.to_switch = switch_id;
        //sm.gsb_medium_id = fsm.medium_id;
        two_stage_mux_inf.first_stages.push_back(sm);
    }
    //for glb stage in imux multistage muxes
    if(is_imux){
        auto const& gsms = imux->glb_stages;
        for(auto const& gsm : gsms){
            t_stage_mux_inf sm;
            auto it = stage_mux_medium_map.find(gsm.mux_name);
            VTR_ASSERT(it != stage_mux_medium_map.end());
            medium_ind = it->second;
            sm.to_node = device_ctx.medium_rr_node_indices[x_coord][y_coord][medium_type][0][medium_ind];//2-gsb
            sm.from_nodes = get_from_node_for_multistage_mux_v2(x_coord, y_coord, chan_details_x, chan_details_y,
                                                                grid, wire_type_sizes, gsm.from_infs, gsb->pbtype_names[0], other_pbpin_to_clbpin_map,
                                                                potential_wires_map, clbpin_medium_map, imux_stage_mux_medium_map, gsb_stage_mux_medium_map, omux_stage_mux_medium_map);
            sm.to_switch = switch_id;
            //sm.gsb_medium_id = fsm.medium_id;
            two_stage_mux_inf.first_stages.push_back(sm);
    ...
    if(is_imux){
        ...
        for (auto const& ssm : ssms) {
            ...
            clb_type = find_pb_type(imux->pbtype_names[0]); //以第一种pb为参考构建
            ...
            sm.from_nodes = get_from_node_for_multistage_mux_v2(x_coord, y_coord, chan_details_x, chan_details_y,
                                                                 grid, wire_type_sizes, ssm.from_infs, imux->pbtype_names[0], 
                                                                 other_pbpin_to_clbpin_map, potential_wires_map, clbpin_medium_map, imux_stage_mux_medium_map, gsb_stage_mux_medium_map, omux_stage_mux_medium_map);
        ...
        //cas stage in gsb
        auto const& csms = gsb->cas_stages;
        for(auto const& csm : csms){
            t_stage_mux_inf sm;
            auto it = stage_mux_medium_map.find(csm.mux_name);
            VTR_ASSERT(it != stage_mux_medium_map.end());
            medium_ind = it->second;
            sm.to_node = device_ctx.medium_rr_node_indices[x_coord][y_coord][medium_type][0][medium_ind];//2-gsb
            sm.from_nodes = get_from_node_for_multistage_mux_v2(x_coord, y_coord, chan_details_x, chan_details_y,
                                                                grid, wire_type_sizes, csm.from_infs, gsb->pbtype_names[0], other_pbpin_to_clbpin_map,
                                                                potential_wires_map, clbpin_medium_map, imux_stage_mux_medium_map, gsb_stage_mux_medium_map, omux_stage_mux_medium_map);
            sm.to_switch = switch_id;
            for (auto const& mux_name : csm.firststage_mux_name) {
                auto it = stage_mux_medium_map.find(mux_name);
                if(it == stage_mux_medium_map.end()){
                    vpr_throw(VPR_ERROR_ARCH, __FILE__, __LINE__, "Error from mux_name %s\n", mux_name.c_str());
                }
...
void compute_two_stage_omux_map(int x_coord, int y_coord,
                                const t_chan_details& chan_details_x, const t_chan_details& chan_details_y,
                                t_omux_inf* omux,
                                const DeviceGrid& grid, t_wire_type_sizes* wire_type_sizes,
                                t_two_stage_mux_map* two_stage_mux_map,
    ...
    auto const& fsms = omux->first_stages;
    auto const& ssms = omux->second_stages;
    two_stage_mux_inf.first_stages.reserve(fsms.size());
    two_stage_mux_inf.second_stages.reserve(ssms.size());

    int switch_id = omux->first_stage_switch_id;
    int medium_type = 1;
    //first stage
    int medium_ind;
    for(auto const& fsm : fsms){
        t_stage_mux_inf sm;
        auto it = omux_stage_mux_medium_map.find(fsm.mux_name);
        VTR_ASSERT(it != omux_stage_mux_medium_map.end());
        medium_ind = it->second;
        sm.to_node = device_ctx.medium_rr_node_indices[x_coord][y_coord][medium_type][0][medium_ind];//1-omux
        sm.from_nodes = get_from_node_for_multistage_omux_v2(x_coord, y_coord, chan_details_x, chan_details_y,
                                                            grid, wire_type_sizes, fsm.from_infs, omux->pbtype_names[0], other_pbpin_to_clbpin_map,
                                                            potential_wires_map, clbpin_medium_map, omux_stage_mux_medium_map);
        sm.to_switch = omux->first_stage_switch_id;
        //sm.gsb_medium_id = fsm.medium_id;
        two_stage_mux_inf.first_stages.push_back(sm);
    }
    ...
    for (auto const& ssm : ssms) {
        t_stage_mux_inf sm;
        auto it = omux_stage_mux_medium_map.find(ssm.mux_name);
        VTR_ASSERT(it != omux_stage_mux_medium_map.end());
        medium_ind = it->second;
        sm.to_node = device_ctx.medium_rr_node_indices[x_coord][y_coord][medium_type][0][medium_ind];//2-gsb
        ...
        sm.from_nodes = get_from_node_for_multistage_omux_v2(x_coord, y_coord, chan_details_x, chan_details_y,
                                                            grid, wire_type_sizes, ssm.from_infs, omux->pbtype_names[0],
                                                            other_pbpin_to_clbpin_map, potential_wires_map, clbpin_medium_map, omux_stage_mux_medium_map);

        for (auto const& mux_name : ssm.firststage_mux_name) {
            auto it = omux_stage_mux_medium_map.find(mux_name);
            if(it == omux_stage_mux_medium_map.end()){
                vpr_throw(VPR_ERROR_ARCH, __FILE__, __LINE__, "Error from mux_name %s\n", mux_name.c_str());
            }
            //VTR_ASSERT(it != stage_mux_medium_map.end());
            medium_ind = it->second;
...
static std::vector<int> get_from_node_for_multistage_mux_v2(int x_coord, int y_coord, const t_chan_details& chan_details_x, const t_chan_details& chan_details_y,
    ...
    for (auto const& same_from : from_mux_inf) {
        ...
        if (same_from.mux_from_type == e_from_type::FT_SEGMENT) {
            ...
            for (int j = 0; j < same_from.from_omux.size(); j++) {
                auto omux_name = same_from.from_omux[j];
                auto it = omux_stage_mux_medium_map.find(omux_name);
                VTR_ASSERT(it != omux_stage_mux_medium_map.end());
                int omux_index = it->second;
                int from_node = get_medium_rr_node_index(device_ctx.medium_rr_node_indices, from_x, from_y, 1, 0, omux_index); //0-IMUX, 1-OMUX, 2-GSB
                if (from_node == OPEN) {
                    continue;
                    VPR_THROW(VPR_ERROR_ROUTE, "find gsb multistage mux from_node OPEN in compute_multistage_mux_map, from_glb_mux = %s.\n", same_from.from_glb[j].c_str());
                }
                medium_from_nodes.push_back(from_node);
            }
        } else if (same_from.mux_from_type == e_from_type::FT_PB) {
        ...
        } else if (same_from.mux_from_type == e_from_type::FT_GLB) {
            if (from_x < 0 || from_x > grid.width() - 1 || from_y < 0 || from_y > grid.height() - 1)
                continue;

            for (int j = 0; j < same_from.from_glb.size(); j++) {
                string glb_name = same_from.from_glb[j];
                auto it = imux_stage_mux_medium_map.find(glb_name);
                VTR_ASSERT(it != imux_stage_mux_medium_map.end());
                int glb_index = it->second;
                int from_node = get_medium_rr_node_index(device_ctx.medium_rr_node_indices, from_x, from_y, 0, 0, glb_index); //0-IMUX, 1-OMUX, 2-GSB
                if (from_node == OPEN) {
                    continue;
                    VPR_THROW(VPR_ERROR_ROUTE, "find gsb multistage mux from_node OPEN in compute_multistage_mux_map, from_glb_mux = %s.\n", same_from.from_glb[j].c_str());
                }
        ...
        } else if (same_from.mux_from_type == e_from_type::FT_CAS){
            for (int j = 0; j < same_from.from_cas.size(); j++) {
                string cas_name = same_from.from_cas[j];
                auto it = gsb_stage_mux_medium_map.find(cas_name);
                VTR_ASSERT(it != gsb_stage_mux_medium_map.end());
                int cas_index = it->second;
                int from_node = get_medium_rr_node_index(device_ctx.medium_rr_node_indices, from_x, from_y, 2, 0, cas_index); //0-IMUX, 1-OMUX, 2-GSB
                if (from_node == OPEN) {
                    continue;
                    VPR_THROW(VPR_ERROR_ROUTE, "find gsb multistage mux from_node OPEN in compute_multistage_mux_map, from_cas_mux = %s.\n", same_from.from_cas[j].c_str());
                }
                medium_from_nodes.push_back(from_node);
            }
        }
            ...
            for (size_t y_coord = 0; y_coord < grid.height(); y_coord++) {
                ...
                if (is_gsb_arch) {
                    ...
                    compute_two_stage_mux_map(x_coord, y_coord, chan_details_x, chan_details_y,
                                              &gsb, &imux, grid, &wire_type_sizes, gsb_two_stage_map, potential_wires_map,
                                              other_pbpin_to_clbpin_map, gsb_stage_mux_medium_map, imux_stage_mux_medium_map, omux_stage_mux_medium_map, clbpin_medium_map,
                                              wire_to_arch_ipin_switch,false);

                    VTR_LOGV(verbose, "***********************IMUX two stage mux map compute *****************************\n");
                    compute_two_stage_mux_map(x_coord, y_coord, chan_details_x, chan_details_y,
                                              &gsb, &imux, grid, &wire_type_sizes, imux_two_stage_map, potential_wires_map,
                                              other_pbpin_to_clbpin_map, gsb_stage_mux_medium_map, imux_stage_mux_medium_map, omux_stage_mux_medium_map, clbpin_medium_map,
                                              wire_to_arch_ipin_switch, true);
                }
            }
        }
    }
    return std::make_pair(gsb_two_stage_map, imux_two_stage_map);
}

t_two_stage_mux_map* alloc_and_load_omux_two_stage_map(const t_chan_details& chan_details_x,
                                                       const t_chan_details& chan_details_y,
                                                       const DeviceGrid& grid,
                                                       std::vector<t_omux_inf> omux_fs,
                                                       const std::vector<t_segment_inf>& segment_inf,
                                                       t_chan_width* nodes_per_chan,
                                                       const std::map<std::string, std::vector<std::map<int, int>>>& other_pbpin_to_clbpin_map,
                                                       const std::map<std::string, int>& omux_stage_mux_medium_map,
    ...
    /* sparse array that will contain switch block connections */
    t_two_stage_mux_map* omux_two_stage_map = new t_two_stage_mux_map;
    ...
    /* iterate over all the gsb specified in the architecture */
    for (int i_gsb = 0; i_gsb < (int)omux_fs.size(); i_gsb++) {
        t_omux_inf omux = omux_fs[i_gsb];
        ...
        for (size_t x_coord = 0; x_coord < grid.width(); x_coord++) {
            for (size_t y_coord = 0; y_coord < grid.height(); y_coord++) {
                t_type_ptr type = grid[x_coord][y_coord].type;
                bool is_gsb_arch = false;
                for (auto pb_name : omux.pbtype_names) {
                    if (0 == strcmp(type->name, pb_name.c_str())) {
                        is_gsb_arch = true;
                        break;
                    }
                ...
                if (is_gsb_arch) {
                    ...
                    /* now we iterate over all the potential side1->side2 connections */
                    VTR_LOGV(verbose, "***********************GSB two stage mux map compute *****************************\n");
                    compute_two_stage_omux_map(x_coord, y_coord, chan_details_x, chan_details_y,
                                              &omux,  grid, &wire_type_sizes, omux_two_stage_map, potential_wires_map,
                                              other_pbpin_to_clbpin_map, omux_stage_mux_medium_map, clbpin_medium_map);
